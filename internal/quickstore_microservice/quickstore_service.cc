...
QuickstoreService::Create(
  ...
  auto status_or_mako_client = mako::NewMakoClient();
  if (!status_or_mako_client.ok()) {
    return std::move(status_or_mako_client).status();
  }
  return {absl::make_unique<QuickstoreService>(
      shutdown_queue, std::move(status_or_mako_client).value())};
}
