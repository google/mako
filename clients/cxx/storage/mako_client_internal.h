...
mako::google3_storage::Storage* NewMakoClient(absl::string_view hostname,
                                                  std::string* error) {
  helpers::StatusOr<std::unique_ptr<mako::google3_storage::Storage>>
      status_or_client = mako::NewMakoClient(hostname);
  if (status_or_client.ok()) {
    return std::move(status_or_client).value().release();
  }
  *error = helpers::StatusToString(status_or_client.status());
  return nullptr;
}
