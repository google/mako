...
inline bool IsOK(const QuickstoreOutput& output) {
  return output.status() == QuickstoreOutput::SUCCESS;
}
...
class Quickstore {
 public:
  ...
  explicit Quickstore(const QuickstoreInput& input)
  ...
  // It does not take ownership of the Storage client object.
  Quickstore(const QuickstoreInput& input, Storage* storage)
  ...
  virtual QuickstoreOutput Store();
